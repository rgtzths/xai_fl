...
X_test = resample(x_test, n_samples=n_samples, random_state=42, stratify=y_test)
print(f"X_test shape: {X_test.shape}")

for model_path in models:
    print(f"Model: {model_path.split('/')[-1]}")
    model = tf.keras.models.load_model(model_path)
    df_res = XAI[args.x](x_train, y_train, X_test, y_test, model)
    model_name = model_path.split("/")[-1].split(".")[0]
    df_res.to_csv(folder / f"{model_name}.csv", index=False)